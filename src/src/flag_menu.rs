...
pub const FLAGS: [(&'static str, Flag); 226] =
    [("Has seen Helmaroc arriving at Outset", HAS_SEEN_HELMAROC_ARRIVING_AT_OUTSET),
     ...
     ("Talked to Olivio at Forest Haven", TALKED_TO_OLIVIO_IN_FOREST_HAVEN),
     ...
     ("Talked to Aldo in Forest Haven", TALKED_TO_ALDO_IN_FOREST_HAVEN),
     ...
     ("Talked to KoRL after leaving Forest Haven", TALKED_TO_KORL_AFTER_LEAVING_FH),
     ("Talked to KoRL on DRI", TALKED_TO_KORL_ON_DRI),
     ("Watched Forest Haven intro Cutscene", WATCHED_FOREST_HAVEN_INTRO_CUTSCENE),
     ...
     ("Talked to Rown in Forest Haven",TALKED_TO_ROWN_IN_FOREST_HAVEN),
     ("Talked to Irch in Forest Haven", TALKED_TO_IRCH_IN_FOREST_HAVEN),
     ("Talked to Drona in Forest Haven", TALKED_TO_DRONA_IN_FOREST_HAVEN),
     ("Talked to Linder in Forest Haven", TALKED_TO_LINDER_IN_FOREST_HAVEN),
     ("Talked to Oakin in Forest Haven", TALKED_TO_OAKIN_IN_FOREST_HAVEN),
     ("Talked to Hollo in store in Forest Haven", TALKED_TO_HOLLO_IN_STORE_IN_FOREST_HAVEN),
     ("Talked to Elma outside Forest Haven", TALKED_TO_ELMA_OUTSIDE_FOREST_HAVEN),
     ("Showed Orca Knight's Crest", SHOW_ORCA_KNIGHTS_CREST),
     ("Watched light bridge in ToTG appear", WATCHED_LIGHT_BRIDGE_IN_TOTG_APPEAR),
     ...
     ("Talked to Deku Tree after Leaf Cutscene", TALKED_TO_DEKU_TREE_AFTER_LEAF_CUTSCENE),
     ("Tetra Gossip Stone text after Bombs", TETRA_GOSSIP_STONE_TEXT_AFTER_BOMBS),
     ...
     ("Watched Light Bridge in ToTG disapper", WATCHED_LIGHT_BRIDGE_IN_TOTG_DISAPPEAR),
     ...
     ("Talked to Nico after Gossip text after bombs", TALKED_TO_NICO_AFTER_GOSIP_TEXT_AFTER_BOMBS),
     ...
     ("Talked to Rito Leader after DRC", TALKED_TO_RITO_LEADER_AFTER_DRC),
     ("Saw Komali in his Room", SAW_KOMALI_IN_HIS_ROOM),
     ("Talked to Makar post Kalle Demos", TALKED_TO_MAKAR_POST_KALLE_DEMOS),
     ...
     ("Watched Farore's Pearl Cutscene", WATCHED_FARORES_PEARL_CUTSCENE),
     ("Leaving Forest Haven with pearl", LEAVING_FOREST_HAVEN_WITH_PEARL),
     ("Talked to Hollo after Forbidden Woods", TALKED_TO_HOLLO_AFTER_FORBIDDEN_WOODS),
     ...
     ("Quill tells us Jabun is hiding at Outset", QUILL_TELLS_US_JABUN_IS_HIDING_AT_OUTSET),
     ("Talked to KoRL after getting bombs", TALKED_TO_KORL_AFTER_GETTING_BOMBS),
     ("Watched Windfall Pirate Cutscene", WATCHED_WINDFALL_PIRATE_CUTSCENE),
     ("Saw Quill Cutscene on DRI", SAW_QUILL_CUTSCENE_ON_DRI),
     ("Talked to Linda on Windfall", TALKED_TO_LINDA_ON_WINDFALL),
     ("Listened to Pirates in Bomb Shop", LISTENED_TO_PIRATES_IN_BOMB_SHOP),
     ("Talked to Rito outside Medli's Room", TALKED_TO_RITO_OUTSIDE_MEDLI_ROOM),
     ...
     ("Picked up statue in ToTG Floor 1", PICKED_UP_STATUE_IN_TOTG_FLOOR_1),
     ("Got Note to Mom", GOT_NOTE_TO_MOM),
     ("Said yes to Baito the first time", SAID_YES_TO_BAITO_FIRST_TIME),
     ...
     ("Entered KoRL after Forbidden Woods", ENTERED_KORL_AFTER_FORBIDDEN_WOODS),
     ("Enter KoRL for the first time / Spawn anywhere", ENTER_KORL_FOR_THE_FIRST_TIME_AND_SPAWN_ANYWHERE),
     ("Cut all ropes holding platfrom in DRC", CUT_ALL_ROPES_HOLDING_PLATFORM_IN_DRC),
     ...
     ("Destroyed vine on door in Forbidden Woods", DESTROY_VINE_ON_DOOR_IN_FORBIDDEN_WOODS),
     ("Talked to KoRL after Forest Haven Cutscene", TALKED_TO_KORL_AFTER_FH_CUTSCENE),
     ("Mighty Darknuts defeated", MIGHTY_DARKNUTS_DEFEATED),
     ("Barrier down", BARRIER_DOWN),
     ("Talked to Rose on Outset when visiting Outset a second time", TALKED_TO_ROSE_WHEN_VISITING_OUTSET_A_SECOND_TIME),
     ...
     ("Talked to Great Fairy on Outset", TALKED_TO_GREAT_FAIRY_ON_OUTSET),
     ("Talked to Zill on Outset West", TALKED_TO_ZILL_ON_OUTSET_WEST),
     ("Talked to Baito outside DRI after DRC", TALKED_BAITO_OUTSIDE_DRI_AFTER_DRC),
     ("Talked to Abe when visiting Outset a second time", TALKED_TO_ABE_WHEN_VISITING_OUTSET_A_SECOND_TIME),
     ...
     ("Talked to Sturgeon when visiting outset a second time", TALKED_TO_STURGEON_WHEN_VISITING_OUTSET_A_SECOND_TIME),
     ("Colors in Hyrule", COLORS_IN_HYRULE),
     ("Watched Courtyard Cutscene", WATCHED_COURTYARD_CUTSCENE),
     ("Entered Hyrule 1 warp and lowered Electrical Barrier", ENTERED_HYRULE_1_WARP_AND_LOWERED_ELECTRICAL_BARRIER),
     ("Talked to KoRL Post-Tower Cutscene", TALKED_TO_KORL_POST_TOWER_CUTSCENE),
     ("Moved Statue in Hyrule", MOVED_STATUE_IN_HYRULE),
     ("Watched FF2 Ganondorf Cutscene", WATCHED_FF2_GANONDORF_CUTSCENE),
     ("Talked to Deku Tree after Farore's Pearl Cutscene", TALKED_TO_DEKU_TREE_AFTER_FARORES_PEARL_CUTSCENE),
     ...
     ("Talked to Sue-Belle when visiting Outset a second time", TALKED_TO_SUE_BELLE_WHEN_VISITING_OUTSET_A_SECOND_TIME),
     ("Watched Medli Dragon Roost Cutscene", WATCHED_MEDLI_DRAGON_ROOST_CUTSCENE),
     ("Talked to locked Pirate Ship Door", TALKED_TO_LOCKED_PIRATE_SHIP_DOOR),
     ...
     ("Entered Grandma's Room durring Endless Night", ENTER_GRANDMAS_ROOM_DURRING_ENDLESS_NIGHT),
     ("Arriving at Outset after bombs", ARRIVING_AT_OUTSET_AFTER_BOMBS),
     ("Dont show Weapons", DONT_SHOW_WEAPONS),
     ("Talked to KoRL leaving Outset before Nayru's Pearl", TALKED_TO_KORL_LEAVING_OUTSET_BEFORE_NAYRUS_PEARL)];
